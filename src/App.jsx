...
export default function App() {
  ...
  // URL param sync (SEO-friendly URLs)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const n = params.get('nationality'); const d = params.get('destination');
    if (n && data.some(r => r.nationality === n)) setNationality(n);
    if (d && data.some(r => r.destination === d)) setDestination(d);
  }, [data]);
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <section className="mx-auto max-w-5xl px-4 py-8">
        <div className="grid gap-4 md:grid-cols-3">
          <div className="md:col-span-2 rounded-2xl bg-white shadow-sm ring-1 ring-gray-200 p-5">
            ...
            <button onClick={handleCheck} className="mt-5 w-full rounded-2xl bg-black px-4 py-3 text-white font-semibold transition hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black">Check Requirements</button>
          ...
          <aside className="rounded-2xl bg-white shadow-sm ring-1 ring-gray-200 p-5">
            ...
            <ul className="mt-3 list-disc pl-5 space-y-1 text-sm text-gray-600">
              ...
              <li>Estimated fee, processing time & stay limit</li>
              <li>Documents checklist to avoid surprises at the border</li>
            </ul>
          </aside>
        </div>
            ...
            {typeof result === 'string' ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  {result.stayLimit && (<span className="text-sm text-gray-600">· Stay limit: <strong>{result.stayLimit} days</strong></span>)}
                </div>
                {!result.allowed && (
                  <div className="text-gray-800"><span className="font-medium">Estimated fee:</span> <Currency value={result.fee} currency={result.currency || 'INR'} /><span className="mx-2">•</span><span className="font-medium">Processing:</span> {result.processing || '—'}</div>
                )}
      ...
      <footer className="border-t bg-white">
        <div className="mx-auto max-w-5xl px-4 py-6 text-xs text-gray-500">This tool is for informational purposes only and may be outdated. Always confirm with the destination’s official immigration website or embassy.</div>
      </footer>
    </main>
  );
}
